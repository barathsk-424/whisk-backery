...
if (process.env.MONGODB_URI) {
  mongoose.connect(process.env.MONGODB_URI)
    ...
    .catch(err => console.warn('⚠️ Artisan Vault (MongoDB) Unavailable:', err.message));
}
...
const isValidEmail = (email) => {
  ...
  if (!match) return { valid: false, message: "Please provide a valid artisan email format." };
  
  const domain = match[1].toLowerCase();
  if (DISPOSABLE_DOMAINS.includes(domain)) {
      return { valid: false, message: "Disposable artisan identities are not permitted for registry." };
  }
...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const { data: existingAdmin } = await supabase.from('admins').select('id').eq('email', userEmail).maybeSingle();
    const { data: existingUser } = await supabase.from('users').select('id').eq('email', userEmail).maybeSingle();

    if (existingAdmin || existingUser) {
      return res.status(400).json({ message: "Artisan already exists in the registry." });
    }
    ...
    console.log(`   → OK: New Artisan registered: ${newUser?.email || userEmail}`);
    res.status(201).json({ message: "Registration successful. Welcome to the community!" });
  ...
  try {
    ...
    if (masterAdmin && masterAdmin.password === password) {
       ...
       const token = jwt.sign(
         { id: masterAdmin.id, email: masterAdmin.email, role: 'admin', source: 'master' },
         JWT_SECRET,
         { expiresIn: "1d" }
       );
       return res.json({ token, user: { id: masterAdmin.id, email: masterAdmin.email, role: 'admin', name: masterAdmin.name } });
    }
    ...
    if (user) {
       if (user.password === password) {
          ...
          const token = jwt.sign(
            { id: user.id, email: user.email, role: user.role || 'user', source: 'general' },
            JWT_SECRET,
            { expiresIn: "1d" }
          );
