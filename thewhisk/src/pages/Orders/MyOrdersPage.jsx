...
const STATUS_COLORS = {
  ...
  preparing:          { bg: 'bg-orange-50',   text: 'text-orange-600', dot: 'bg-orange-400' },
  'out for delivery': { bg: 'bg-purple-50',   text: 'text-purple-600', dot: 'bg-purple-400' },
  delivered:          { bg: 'bg-green-50',    text: 'text-green-600',  dot: 'bg-green-500' },
};

function statusStyle(status = '') {
  return STATUS_COLORS[status.toLowerCase()] || { bg: 'bg-gray-50', text: 'text-gray-600', dot: 'bg-gray-400' };
}
...
export default function MyOrdersPage() {
  ...
  const handleDownload = (order) => {
    const productName = order.products?.name || order.product_name || 'Cake Product';
    const lines = [
      '── THE WHISK – RECEIPT ─────────',
      `Order  : ${order.id}`,
      ...
      '',
      order.address?.line1 ? `Address: ${order.address?.line1}, ${order.address?.city}` : '',
  ...
  const totalSpent = orders.reduce((s, o) => s + (o.total_price || o.price || 0), 0);
    ...
    <div className="min-h-screen pt-24 pb-16 bg-brown-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="font-heading text-4xl font-bold text-primary">🛍️ My Orders</h1>
            <p className="text-brown-400 mt-2">Track all your delicious cake orders in one place.</p>
          </div>
          {orders.length > 0 && (
            <button
              onClick={clearOrders}
              className="px-4 py-2 border border-red-200 text-red-500 bg-red-50 hover:bg-red-100 rounded-xl font-bold text-sm shadow-sm transition-colors self-start sm:self-auto"
            >
              Clear Order History
            </button>
          )}
        ...
        <AnimatePresence mode="popLayout">
          {filtered.length === 0 ? (
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <h3 className="font-heading text-xl font-bold text-primary mb-2">No orders found</h3>
              <p className="text-sm text-brown-400 mb-6 font-medium max-w-sm">
                {filter === 'all' ? "Looks like you haven't placed any orders. Start baking your dream cake!"
            ...
            filtered.map((order, idx) => {
              ...
              const productName = order.products?.name || order.product_name || 'Custom Cake';
              const imageUrl    = order.products?.image || order.image_url || 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=200&h=200&fit=crop';
              ...
              return (
                ...
                >
                  <button className="w-full text-left p-4 sm:p-6" onClick={() => setExpandedId(isOpen ? null : order.id)}>
                    <div className="flex items-center gap-4 sm:gap-5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 flex-wrap sm:flex-nowrap">
                          <div className="min-w-0">
                            <p className="font-bold text-primary leading-tight line-clamp-1 sm:text-lg">
                              {productName}
                              {order.quantity > 1 && (
                                <span className="ml-2 text-xs text-brown-400 font-medium bg-brown-100/50 px-2 py-0.5 rounded-full">x{order.quantity}</span>
                              )}
                            </p>
                          ...
                          <span className={`flex items-center gap-1.5 text-[10px] sm:text-[11px] font-bold px-2.5 py-1 sm:py-1.5 rounded-full shrink-0 ${style.bg} ${style.text} capitalize`}>
                        ...
                        <div className="flex items-center justify-between mt-3 sm:mt-2">
                          ...
                          <p className="font-extrabold text-accent text-sm sm:text-base">₹{orderTotal.toLocaleString()}</p>
                        </div>
                      </div>
                      <motion.span animate={{ rotate: isOpen ? 180 : 0 }} className="text-brown-300 shrink-0 text-lg sm:text-2xl hidden sm:block">
                        ⌄
                      </motion.span>
                    </div>
                  </button>
                        ...
                        <div className="p-4 sm:p-6 space-y-4 bg-brown-50/40">
                          <div className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-brown-50 shadow-sm">
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-brown-400 mt-1">Qty: {order.quantity}</p>
                            </div>
                            <p className="font-bold text-sm text-primary">₹{orderTotal.toLocaleString()}</p>
                          </div>
                          ...
                          {order.address && Object.keys(order.address).length > 0 && (
                            <div className="flex gap-3 text-sm text-brown-500 bg-white p-4 rounded-2xl border border-brown-50 shadow-sm">
                              ...
                              <div>
                                <p className="font-bold text-primary text-xs mb-0.5">Delivery Address</p>
                                <p className="text-xs">{order.address.line1}, {order.address.city} – {order.address.pincode}</p>
                              </div>
                            </div>
                          )}
                          ...
                          <div className="flex gap-3 pt-2">
                            {order.status !== 'delivered' && (
                              <button onClick={() => { localStorage.setItem('lastOrder', JSON.stringify(order)); navigate('/track-order'); }}
