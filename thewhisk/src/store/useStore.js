...
const useStore = create((set, get) => ({
  ...
  initiateRazorpayPayment: async (amount, orderId) => {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
          razorpay_payment_id: "pay_artisan_" + Math.random().toString(36).substr(2, 9),
  ...
  fetchOrders: async () => {
    ...
    try {
      const { success, orders, error } = await getOrdersAPI({ user_id: uID, email: get().user?.email });
  ...
  deleteOrder: async (orderId) => {
    ...
    if (orderId && !orderId.toString().startsWith('ord-') && !orderId.toString().includes('mock')) {
      try {
        ...
        console.warn("DB deletion failed, but hidden from session UI:", err.message);
      }
    } else {
  ...
  clearOrders: async () => {
    const { user } = get();
    if (!user) return;
    if (!window.confirm("Are you sure you want to clear your entire order history from the Artisan Vault?")) return;
    ...
    try {
      ...
      const { error } = await supabase.from('orders').delete().eq('user_id', uID);
  ...
  fetchSavedDesigns: async () => {
    ...
    if (!uID) return;
    try {
      const { data, error } = await supabase.from('saved_designs').select('*').eq('user_id', uID).order('created_at', { ascending: false });
      if (error) throw error;
  ...
  ensureUserProfile: async () => {
    ...
    try {
      const profileData = {
        id: user.id || user.sub,
        email: user.email,
        name: user.user_metadata?.full_name || user.email?.split('@')[0] || 'Artisan Member'
      };
  ...
  placeOrder: async (orderMetadata) => {
    ...
    try {
      const orderPayload = {
        ...
        customer_name: orderMetadata.delivery_details?.name || user.user_metadata?.full_name || 'Artisan Guest',
        items: cart, 
        ...
        delivery_date: orderMetadata.delivery_date || new Date().toISOString().split('T')[0],
        ...
        revenue_category: orderMetadata.revenue_category || (getCartTotal() > 3000 ? 'Premium Artisan' : 'Standard Collection'),
  ...
  saveCakeDesign: async (designData) => {
    ...
    try {
      const { data, error } = await supabase.from('saved_designs').insert([{ user_id: user.id, ...designData }]).select().single();
      if (error) throw error;
  ...
  addToCart: async (product, quantity = 1) => {
    ...
    let newCart = existing 
      ? cart.map(i => (i.id === product.id && i.selectedSize === product.selectedSize) 
  ...
  removeFromCart: (productId, selectedSize) => {
    const newCart = get().cart.filter(i => !(i.id === productId && i.selectedSize === selectedSize));
    set({ cart: newCart });
    localStorage.setItem('cart', JSON.stringify(newCart));
  },
  ...
  updateCartQuantity: (productId, selectedSize, newQty) => {
     ...
     const newCart = get().cart.map(i => (i.id === productId && i.selectedSize === selectedSize) ? { ...i, quantity: newQty } : i);
     set({ cart: newCart });
     localStorage.setItem('cart', JSON.stringify(newCart));
  },

  toggleCart: () => set(state => ({ cartOpen: !state.cartOpen })),
  getCartCount: () => get().cart.reduce((count, item) => count + (item.quantity || 1), 0),
  getCartTotal: () => get().cart.reduce((total, item) => total + ((item.price || 0) * (item.quantity || 1)), 0),
  ...
  logout: async () => {
    localStorage.removeItem("token");
    set({ user: null, profile: null, token: null, isAuthenticated: false, isAdmin: false, orders: [], savedDesigns: [] });
  ...
  addReview: async (reviewData) => {
    ...
    try {
      const { data, error } = await supabase.from('reviews').insert([{ ...reviewData, user_id: user.id, user_name: user.name || user.email.split('@')[0] }]).select().single();
      if (error) throw error;
